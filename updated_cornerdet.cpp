...
void opticalFlow(unsigned char ref[H][W], unsigned char next[H][W], int inputquad[maxFeatures][2], int outputquad[maxFeatures][2], int status[maxFeatures])
{
...
// Function to get cofactor of A[p][q] in temp[][]. n is current
// dimension of A[][]
void getCofactor2(int A[][8],int temp[][8],int p,int q,int n)
{
    int i = 0, j = 0;
...
/* Recursive function for finding determinant of matrix.
   n is current dimension of A[][]. */
int determinant2(int A[][8], int n)
{
    int D = 0; // Initialize result
    ...
    int temp[8][8]; // To store cofactors
    ...
    {
        // Getting Cofactor of A[0][f]
        getCofactor2(A, temp, 0, f, n);
        D += sign * A[0][f] * determinant2(temp, n - 1);
...
// Function to get adjoint of A[N][N] in adj[N][N].
void adjoint2(int A[][8],int adj[][8], int N)
{
    if (N == 1)
    {
        adj[0][0] = 1;
    ...
    for (int i=0; i<N; i++)
    {
        for (int j=0; j<N; j++)
        {
            ...
            // Interchanging rows and columns to get the
            // transpose of the cofactor matrix
            adj[j][i] = sign*determinant2(temp,N-1) ;
        }
    }
}
...
// Function to calculate and store inverse, returns false if
// matrix is singular
bool inverse2(int A[][8], float inverse[][8])
{
    // Find determinant of A[][]
    int det = determinant2(A, 8);
    if (det == 0)
    {
        //cout << "Singular matrix, can't find its inverse";
        return false;
