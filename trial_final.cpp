...
/*********************************************************************/
      ...
      outputquad[i][0] = res[0][0]*vl + inputquad[i][0]; //storing final values of the corner in output quad
      outputquad[i][1] = res[1][0]*vl + inputquad[i][1];
...
void getCofactor_2(float A[][8],float temp[][8],int p,int q,int n)
{
    int i = 0, j = 0;
...
/* Recursive function for finding determinant of matrix.
   n is current dimension of A[][]. */
float determinant_2(float A[][8],int n)
{
    ...
    float temp[8][8]; // To store cofactors
    ...
    {
        // Getting Cofactor of A[0][f]
        getCofactor_2(A,temp,0,f,n);
        D += sign * A[0][f] * determinant_2(temp,n-1);
...
// Function to get adjoint of A[N][N] in adj[N][N].
void adjoint_2(float A[][8],float adj[][8])
{
    ...
    for (int i=0; i<8; i++)
    {
        for (int j=0; j<8; j++)
        {
            ...
            // Interchanging rows and columns to get the
            // transpose of the cofactor matrix
            adj[j][i] = (sign)*(determinant_2(temp,7)) ;
        }
    }
}
...
// Function to calculate and store inverse, returns false if
// matrix is singular
bool inverse_2(float A[][8], float inverse[][8])
{
    // Find determinant of A[][]
    float det = determinant_2(A, 8);
    if (det == 0)
    {
...
// mat1[][] and mat2[][], and
// stores the result in res[][]
void multiply_p(float mat1[8][8],
              float mat2[8],
              float res[8])
{
    int i, j, k;
...
void getPerspectiveTransform(int inputquad[maxFeatures][2], float outputquad[maxFeatures][2], float transfMat[3][3])
{
  float U[8] = {0};
  float C[8] = {0}; //trans mat
  float X[8][8] = {0};
  ...
  if(!inverse_2(X, X_inverse))
  {
  	cout<<err<<endl;//storing type of error for help in debugging
        return;
  }
  multiply_p(X_inverse, U, C);
...
void warp_perspective(unsigned char next[H][W], unsigned char transframe[H][W], float transmat[][3])
{
  int x,y;
...
int main()
{
    ...
    while(1)
    {
      ...
      // If there is no error in feature matching
        
		getPerspectiveTransform(inputQuad, outputQuad, transfMat);
		warp_perspective(matFrame, transFrame, transfMat );
